...
#----------------------------------------------------------------------------------------------------------------
#write to BD
conn = MySQLdb.connect(host= "localhost",user="root",passwd="",db="TnH")
x1 = conn.cursor()
...
try:
    if x == 205:
       ...
       msg = "telegram-send '" + "D:" + date[-8:-3] + " t:" + temp[0:-1] + " h:" + hum[0:-1] + "'"
       print (date)
       telegram = subprocess.Popen(msg, stdout=subprocess.PIPE, shell=True)
#       if date[-5:-4] == '0':
#        msg = "telegram-send '" + "D:" + date[-8:-3] + " t:" + temp[0:-3] + " h:" + hum[0:-3] + "'"
#        print (msg)
#        telegram = subprocess.Popen(msg, stdout=subprocess.PIPE, shell=True)
       conn.commit()
    if x == 204:
